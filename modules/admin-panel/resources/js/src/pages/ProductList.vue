<template>
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">📦 لیست محصولات</h1>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            <div v-for="product in products" :key="product.id" class="bg-white shadow-md rounded-lg p-4">
                ...
                <div class="mt-4 flex justify-between">
                    <button @click="editProduct(product.id)" class="bg-blue-500 text-white px-4 py-2 rounded">ویرایش
                    </button>
                    <button @click="doDeleteProduct(product.id)" class="bg-red-500 text-white px-4 py-2 rounded">حذف
                    </button>
                </div>
            </div>
        </div>
